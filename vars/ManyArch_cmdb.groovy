...
def call( isDeploy=false) {
    node {
        ...
        def dockerfilename="Dockerfile.light"
         ...
         try {
            parallel(
                ...
                "build & deploy": {
                    ...
                    def amd64_image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-cmdb-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    def amd64_image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-cmdb-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    def amd64_image_runtime = "image.rightcloud.com.cn/${namespace}/rightcloud-cmdb-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    def amd64_image_runtime_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-cmdb-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    
                    def arm64_image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-cmdb-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    def arm64_image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-cmdb-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    def arm64_image_runtime = "image.rightcloud.com.cn/${namespace}/rightcloud-cmdb-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    def arm64_image_runtime_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-cmdb-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    ...
                    stage("builder") {
                        ...
                        def build_args = " --network=host --build-arg CMDB_PROJECT=./app_component --build-arg PARENT_PROJECT=./parent  --build-arg COMPILER_IMAGE=swr.cn-east-3.myhuaweicloud.com/distroless/rightcloud-lib:4.2-jenkins "
                            ...
                            dir("/home/jenkins/workspace/") {
                              // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                                  ...
                                  def build_arm64_image = {
                                      ...
                                      sh "docker build -t ${arm64_image_name_huaweiyun} --no-cache --target mainjar -f app_component/docker/${dockerfilename} . ${build_args}"
                                    //   sh "docker push ${arm64_image_name}"
                                      sh "docker build -t ${arm64_image_runtime_huaweiyun} --no-cache --target runtime -f app_component/docker/${dockerfilename} . ${build_args}"
                                  ...
                                  def build_amd64_image = {
                                      sh "docker run --rm --privileged swr.cn-east-3.myhuaweicloud.com/distroless/qemu-user-static:register || :"
                                      sh "docker build -t ${amd64_image_name_huaweiyun} --no-cache --target mainjar -f app_component/docker/${dockerfilename} . ${build_args}"
                                    //   sh "docker push ${amd64_image_name_huaweiyun}"
                                      sh "docker build -t ${amd64_image_runtime_huaweiyun} --no-cache --target runtime -f app_component/docker/${dockerfilename} . ${build_args}"
                                    ...
                                    //   sh "docker tag ${amd64_image_runtime} ${amd64_image_runtime_huaweiyun}"
                                      sh "docker push ${amd64_image_runtime_huaweiyun}"
                                      sh "echo '---------镜像外网拉取X86-------tag：${amd64_image_name_huaweiyun}'"
                                      sh "echo '---------镜像外网拉取X86-------tag：${amd64_image_runtime_huaweiyun}'"
                                  }
