...
def call(isDeploy = false) {
    node {
        ...
        def image_name = "image.rightcloud.com.cn/${namespace}/third-party:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
        def image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/third-party:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    ...
                    dir('/home/jenkins/workspace') {
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            if (params.BUILD_CPU_ARCH=="ARM64"){
                                ...
                                sh "docker build -t ${db_image_name_huaweiyun} --no-cache -f third-party/docker/Dockerfile.db.arm64 . ${build_args}"
                ...
                container('docker') {
                    dir('/home/jenkins/workspace') {
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            ...
                            if (params.BUILD_CPU_ARCH=="ARM64"){
                                sh "docker run --rm --privileged swr.cn-east-3.myhuaweicloud.com/distroless/qemu-user-static:register || :"
                                sh "docker build -t ${image_name} --no-cache -f third-party/docker/${dockerfilename} . ${build_args}"
