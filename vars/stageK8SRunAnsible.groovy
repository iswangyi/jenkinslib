def call(k8server, namespace, version, checkout=false)  {
    ...
    dir(dirname) {
        ...
        stage("run ansible") {
            sh " sed -i 's|\$ansibleTag|'${version}'|g'   rightcloud/app/rightcloud-ansible-template.yaml "
            sh " sed -i 's|\$namespace|'${namespace}'|g' rightcloud/app/rightcloud-ansible-template.yaml"

            sh "kubectl -s ${k8server} apply -f  ./rightcloud/app/rightcloud-ansible-template.yaml"
        }
    }
