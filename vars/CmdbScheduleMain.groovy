...
//        def kubectl = getKubectl(getK8sEnv(namespace))
        ...
        try {
            ...
            def image_name = "image.rightcloud.com.cn/${namespace}/cmdb-scheduler:v.${branchname}.${env.BUILD_NUMBER}"
            ...
            stage("build") {
                ...
                def build_args = " --build-arg SCHEDULE_PROJECT=./  "
            ...
            if (isDeploy) {
                stageK8ServiceDeploy("cmdb-scheduler", namespace, kubectl, image_name, false, 'cmdb')
            }
            //     }
            // )
        } catch (e) {
