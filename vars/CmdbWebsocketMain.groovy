...
//        def kubectl = getKubectl(getK8sEnv(namespace))
        ...
        try {
            ...
            def image_name = "image.rightcloud.com.cn/${namespace}/cmdb-websocket:v.${branchname}.${env.BUILD_NUMBER}"
            stage("build") {
                // 获取Dockerfile
                ...
                def build_args = " --build-arg WEBSOCKET_PROJECT=./  "
            ...
            if (isDeploy) {
                stageK8ServiceDeploy("cmdb-websocket", namespace, kubectl, image_name, false, 'cmdb')
            }
            //     }
            // )
        } catch (e) {
