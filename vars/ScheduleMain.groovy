        ...
        def kubectl = getKubectl(getK8sEnv(namespace))
         ...
         try {
            ...
            //     "build & deploy": {
                    ...
                    def image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-schedule:v.${branchname}.${env.BUILD_NUMBER}"
                    ...
                    stage("build") {
                        ...
                        container('docker') {
                            // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                                sh "docker build -t ${image_name} --no-cache -f docker/Dockerfile . ${build_args}"
                    ...
                    if (isDeploy) {
                        stageK8ServiceDeploy("schedule", namespace, kubectl, image_name, false)
                    }
