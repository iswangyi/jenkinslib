...
def call() {
    node {
        ...
        def host_dir_path = "/cloudStar/cmp397"
            ...
            stage ("handle images and package") {
                sshagent(['ssh-key-10.69.0.234-aio-dev']) {
                    ...
                    if (gateway_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|GATEWAY_TAG=.*|'GATEWAY_TAG=${params.gateway_image}'|g' ${host_file} \" "
                    }
