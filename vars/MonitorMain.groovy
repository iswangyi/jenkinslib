...
//                },
                "build & deploy": {
                    ...
                    def image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}:v.${branchname}.${env.BUILD_NUMBER}"
                    ...
                    stage("build") {
                        ...
                        def  build_args = " --network=host --build-arg   PROJECT=./ "

                        container('docker') {
                            // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {

                                sh "docker build -t ${image_name} --no-cache -f monitor-${SERVER_OR_AGENT}/docker/Dockerfile . ${build_args}"
