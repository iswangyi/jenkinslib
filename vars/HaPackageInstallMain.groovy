...
def call() {
    node {
        ...
        def deploy_dir="/cloudStar/ha-deployment"
        def host_file="${deploy_dir}/host"

        def now = new Date().format("yyyyMMddHHmm")
        def tgz_name = "rightcloud-cmp-ha-installer-${release_version}-${now}.tar.gz"
        def host_path = "/cloudStar/ha-deployment"
        def host_tgz_path = "/cloudStar/tmp/${tgz_name}"
        def baidu_path = "/Release-transfer-standby/rightcloud-cmp/ha"
            ...
            stage ("handle images and package") {
                sshagent(['ssh-key-10.69.0.234-aio-dev']) {
                    ...
                    if (server_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|^SERVER_TAG=.*|'SERVER_TAG=${params.server_image}'|g' ${host_file} \" "
                    }
                    if (ansible_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|ANSIBLE_TAG=.*|'ANSIBLE_TAG=${params.ansible_image}'|g' ${host_file} \" "
                    }
                    if (adapter_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|ADAPTER_TAG=.*|'ADAPTER_TAG=${params.adapter_image}'|g' ${host_file} \" "
                    }
                    if (console_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|CONSOLE_TAG=.*|'CONSOLE_TAG=${params.console_image}'|g' ${host_file} \" "
                    }
                    if (schedule_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|SCHEDULE_TAG=.*|'SCHEDULE_TAG=${params.schedule_image}'|g' ${host_file} \" "
                    }
                    if (monitor_server_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|MONITOR_SERVER_TAG=.*|'MONITOR_SERVER_TAG=${params.monitor_server_image}'|g' ${host_file} \" "
                    }
                    if (monitor_agent_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|MONITOR_AGENT_TAG=.*|'MONITOR_AGENT_TAG=${params.monitor_agent_image}'|g' ${host_file} \" "
                    }
                    if (openapi_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|OPENAPI_TAG=.*|'OPENAPI_TAG=${params.openapi_image}'|g' ${host_file} \" "
                    }
                    if (db_migration_image.trim()) {
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" sed -i 's|DB_MIGRATION_TAG=.*|'DB_MIGRATION_TAG=${params.db_migration_image}'|g' ${host_file} \" "
                    }
                    ...
                    if (ON_OR_OFF.trim() == "online") {
                        ...
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" cd ${deploy_dir} && ./download_images.sh && cd /cloudStar && tar zcf ${host_tgz_path} ha-deployment --exclude=ha-deployment/.git \" "
                    }else {
