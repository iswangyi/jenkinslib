...
//    def kubectl = getKubectl(k8sEnv)
    ...
    node {
        ...
        try {
            ...
            def image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-console:v.${branchname}.${env.BUILD_NUMBER}"
...
//                }
                stage('build console image') {
                    ...
                    def third_party_build_args = "  --network=host --build-arg CONTAINER_PROJECT=./rightcloud-console-container " +
                        ...
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            dir('/home/jenkins/workspace') {
                                ...
                                if (params.BUILD_CPU_ARCH=="ARM64"){
                                    ...
                                    sh "docker build -t ${image_name_huaweiyun} --no-cache -f rightcloud-console/docker/Dockerfile.arm64 . ${build_args}"
                                ...
                                if (params.BUILD_CPU_ARCH=="AMD64"){
                                    sh "docker build --platform=linux/amd64 -t ${image_name_huaweiyun} --no-cache -f rightcloud-console/docker/Dockerfile . ${build_args}"
                                    // sh "docker push ${image_name}"
                                ...
                                // if (params.BUILD_CPU_ARCH=="AMD64_ARM64"){
                                //     sh "docker build --platform=linux/amd64 -t ${image_name} --no-cache -f rightcloud-console/docker/Dockerfile  . ${build_args}"
                                //     sh "docker push ${image_name}"
                                //     sh "echo '>>>>>>>>>AMD64镜像构建完成并上传成功，AMD镜像号为：${image_name}'"
                                //     sh "docker run --rm --privileged swr.cn-east-3.myhuaweicloud.com/distroless/qemu-user-static:register || :"
                                //     sh "docker build -t ${image_name} --no-cache -f rightcloud-console/docker/Dockerfile.arm64 . ${build_args}"
                                //     sh "docker push ${image_name}"
                                //     sh "echo '>>>>>>>>>ARM64镜像构建完成并上传成功，ARM镜像号为：${image_name}'"
