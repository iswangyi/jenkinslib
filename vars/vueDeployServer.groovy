def call(k8sserver, namespace, dbmigration, version, checkout=false)  {
    ...
    dir(dirname) {
        ...
        stage("run server") {
            ...
            def kubectl="kubectl --kubeconfig=/home/kubectl-cer/config"

            sh "${kubectl} apply -f  ./rightcloud/app/rightcloud-server-template.yaml"

                stage("checking dbmiration status") {
                    timeout(time: 180, unit: 'SECONDS') {
                def dbmigrationstatus= sh(script: "pod=\$(${kubectl} -n ${namespace} get pod --sort-by=.metadata.creationTimestamp | grep -v Evicted | grep rightcloud-server| awk '{print \$1}'|tail -1);${kubectl} -n ${namespace} get pod \${pod} --template \'{{range .status.initContainerStatuses}} {{.state.terminated.exitCode}}{{end}}\'", returnStdout: true).trim()

                while (dbmigrationstatus.trim() == '<no value>') {
                    dbmigrationstatus = sh(script: "pod=\$(${kubectl} -n ${namespace} get pod --sort-by=.metadata.creationTimestamp | grep -v Evicted | grep rightcloud-server|awk '{print \$1}'|tail -1);${kubectl} -n ${namespace} get pod \${pod} --template \'{{range .status.initContainerStatuses}} {{.state.terminated.exitCode}}{{end}}\'", returnStdout: true).trim()
                }

                if (dbmigrationstatus.trim() != "0") {
                            def dbmigrationmsg = sh(script: "pod=\$(${kubectl} -n ${namespace} get pod --sort-by=.metadata.creationTimestamp | grep -v Evicted | grep rightcloud-server|awk '{print \$1}'|tail -1);${kubectl} -n ${namespace} logs \${pod} -c dbmigration", returnStdout: true).trim()
