        ...
        try {
            stage ("save all images") {
                sshagent(['ssh-key-10.69.0.234-aio-dev']) {
                    ...
                    if (openapi_image) {
                        tag = "OPENAPI_TAG=${openapi_image}"
                        sh "ssh -T -o StrictHostKeyChecking=no -l root ${BUILD_SERVER} \" cd /cloudStar/upgrade && docker pull ${openapi_image} && docker save ${openapi_image} | gzip > openapi.tgz && echo ${tag} >> /cloudStar/upgrade/.version_info \" "
                        count ++
                    }
