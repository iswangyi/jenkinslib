...
def call(dockerserver, repo, branchortag, namespace, istag, tagprefix=null, checkout=false) {
    ...
    dir(dirname) {
        ...
        def rightcloudadapterrepo = "${projecgroup}/rightcloud-adapter"

        stage("build schedule image") {
            projectname = "image.rightcloud.com.cn/${namespace}/rightcloud-schedule"
...
//            sh " sed -i 's/\$branchName/'${branchortag}'/g' docker/full_container_build/Dockerfile  "
            ...
            dockerBuildPush(fulltag,
                            ...
                            "--build-arg RIGHTCLOUDPROJECT=${rightcloudrepo} --build-arg RIGHTCLOUDADAPTERPROJECT=${rightcloudadapterrepo} --build-arg PROJECT=${repo} --build-arg BRANCH=${branchortag}")
