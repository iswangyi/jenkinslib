...
//    def kubectl = getKubectl(k8sEnv)
    ...
    node {
        ...
        def SERVER_OR_AGENT = env.SERVER_OR_AGENT
        println(env.SERVER_OR_AGENT)
        println(SERVER_OR_AGENT)
...
//                },
                "build & deploy": {
                    ...
                    // def image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    // def image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    // def image_runtime = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
                    // def image_runtime_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"


                    def branchname = "${PARAM_GIT_TAGS}"
                    def amd64_image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-main:v.${branchname}.amd64.${env.BUILD_NUMBER}"
                    def amd64_image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-main:v.${branchname}.amd64.${env.BUILD_NUMBER}"
                    def amd64_image_runtime = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.amd64.${env.BUILD_NUMBER}"
                    def amd64_image_runtime_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.amd64.${env.BUILD_NUMBER}"
                    
                    def arm64_image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-main:v.${branchname}.arm64.${env.BUILD_NUMBER}"
                    def arm64_image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-main:v.${branchname}.arm64.${env.BUILD_NUMBER}"
                    def arm64_image_runtime = "image.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.arm64.${env.BUILD_NUMBER}"
                    def arm64_image_runtime_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.arm64.${env.BUILD_NUMBER}"
                    ...
                    stage("build") {
                        ...
                        def  build_args = " --network=host --build-arg   PROJECT=./app_component --build-arg PARENT_PROJECT=./parent  --build-arg COMPILER_IMAGE=swr.cn-east-3.myhuaweicloud.com/distroless/rightcloud-lib:4.2-jenkins "
                        ...
                        container('docker') {
                            dir("/home/jenkins/workspace/") {
                                ...
                                //     sh "docker build -t ${image_name} --no-cache --target mainjar -f app_component/monitor-${SERVER_OR_AGENT}/docker/${dockerfilename} . ${build_args}"
                                //     sh "docker push ${image_name}"
                                //     sh "docker build -t ${image_runtime} --no-cache --target runtime -f app_component/monitor-${SERVER_OR_AGENT}/docker/${dockerfilename} . ${build_args}"
                                ...
                                // }
                                  ...
                                  def build_arm64_image = {
                                      sh "docker run --rm --privileged swr.cn-east-3.myhuaweicloud.com/distroless/qemu-user-static:register || :"
                                      sh "docker build -t ${arm64_image_name} --no-cache --target mainjar -f app_component/monitor-${SERVER_OR_AGENT}/docker/Dockerfile.light.arm . ${build_args}"
                                      sh "docker push ${arm64_image_name}"
                                      sh "docker build -t ${arm64_image_runtime} --no-cache --target runtime -f app_component/monitor-${SERVER_OR_AGENT}/docker/Dockerfile.light.arm . ${build_args}"
                                  ...
                                  def build_amd64_image = {
                                    //   sh "docker run --rm --privileged swr.cn-east-3.myhuaweicloud.com/distroless/qemu-user-static:register || :"
                                      sh "docker build -t ${amd64_image_name} --no-cache --target mainjar -f app_component/monitor-${SERVER_OR_AGENT}/docker/Dockerfile.light.jenkins . ${build_args}"
                                      sh "docker push ${amd64_image_name}"
                                      sh "docker build -t ${amd64_image_runtime} --no-cache --target runtime -f app_component/monitor-${SERVER_OR_AGENT}/docker/Dockerfile.light.jenkins . ${build_args}"
                    ...
                    if (isDeploy) {
                        stageK8ServiceDeploy("monitor-${SERVER_OR_AGENT}", namespace, kubectl, image_name, false)
                    }
