...
def call(isDeploy = false) {
    node {
        properties([parameters([
                ...
                choice(name: "BUILD_CPU_ARCH", choices: 'ARM64\nAMD64\nAMD64_ARM64', description: 'cpu架构', defaultValue: "ARM64"),
        ...
        parallel(
                ...
                "clone backup": {
                    dir('/home/jenkins/workspace/backup') {
                        checkout scm: [$class: 'GitSCM', userRemoteConfigs: [[url: "http://10.182.0.74:8443/rightcloud-modules/cloud-backup/rightcloud-module-cloudbackup.git", credentialsId: 'bdfdd9e4-1bd4-4332-a46e-b78347a61eb2']], branches: [[name: branchname]]], poll: false
                        sh 'rm -rf .git && rm -rf .idea'
                    }
                },
                "clone container": {
                    dir('/home/jenkins/workspace/container') {
                        checkout scm: [$class: 'GitSCM', userRemoteConfigs: [[url: "http://10.182.0.74:8443/rightcloud-modules/cloud-container/rightcloud-module-cloudcontainer.git", credentialsId: 'bdfdd9e4-1bd4-4332-a46e-b78347a61eb2']], branches: [[name: branchname]]], poll: false
                        sh 'rm -rf .git && rm -rf .idea'
                    }
                },
                "clone parent": {
        ...
        def image_name = "image.rightcloud.com.cn/${namespace}/third-party-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
        def image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/third-party-main:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
        def image_runtime = "image.rightcloud.com.cn/${namespace}/third-party-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
        def image_runtime_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/third-party-runtime:v.${branchname}.${arch_bz}.${env.BUILD_NUMBER}"
        def db_image_name = "image.rightcloud.com.cn/${namespace}/third-party-dbmigration:v.${branchname}.${env.BUILD_NUMBER}"
        def db_image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/third-party-dbmigration:v.${branchname}.${env.BUILD_NUMBER}"
        ...
        def build_args = " --network=host --build-arg BACKUP_PROJECT=./backup " +
                ...
                "--build-arg PARENT_PROJECT=./parent  --build-arg COMPILER_IMAGE=swr.cn-east-3.myhuaweicloud.com/distroless/rightcloud-lib:4.2-jenkins " +
                    ...
                    dir('/home/jenkins/workspace') {
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            if (params.BUILD_CPU_ARCH=="ARM64"){
                                ...
                                sh "docker build -t ${db_image_name_huaweiyun} --no-cache --target migration -f third-party/docker/${dockerfilename} . ${build_args}"
                                ...
                                // sh "echo '>>>>>>>>>ARM64镜像构建完成并上传成功，ARM镜像号为：${db_image_name}'"
                                    ...
                                    // sh "docker tag ${db_image_name} ${db_image_name_huaweiyun}"
                                    sh "docker push ${db_image_name_huaweiyun}"
                                    sh "echo '---------镜像外网拉取arm-------tag：${db_image_name_huaweiyun}'"
                            }
                ...
                container('docker') {
                    dir('/home/jenkins/workspace') {
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            ...
                            if (params.BUILD_CPU_ARCH=="ARM64"){
                                sh "docker run --rm --privileged swr.cn-east-3.myhuaweicloud.com/distroless/qemu-user-static:register || :"
                                sh "docker build -t ${image_name_huaweiyun} --no-cache --target mainjar -f third-party/docker/${dockerfilename} . ${build_args}"
                                // sh "docker push ${image_name}"
                                sh "docker build -t ${image_runtime_huaweiyun} --no-cache --target runtime -f third-party/docker/${dockerfilename} . ${build_args}"
                        ...
                        container('kubectl') {
                            dir("/home/jenkins/workspace/kubernetes-yml") {
                                stage('clone yaml template') {
                                    git credentialsId: '507bb8bd-e791-461e-9946-083fab89d03c', url: 'https://123.60.85.138:8443/cloudstar/kubernetes-yml.git', branch: "master"
                                }
                                stage('run in k8s') {
