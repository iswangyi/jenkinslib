         ...
         try {
            // parallel(
                ...
                // "build & deploy": {
                    ...
                    def image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-resource:v.${branchname}.${env.BUILD_NUMBER}"

                    stage("build") {
                        // 获取Dockerfile
                        checkout scm: [$class: 'GitSCM', userRemoteConfigs: scm.userRemoteConfigs, branches: [[name: branchname]]], poll: false
                        def build_args = " --build-arg RESOURCE_PROJECT=./  "
                    ...
                    if (isDeploy) {
                        stageK8ServiceDeploy("resource", namespace, kubectl, image_name, false)
                    }
            //     }
            // )
         } catch (e) {
