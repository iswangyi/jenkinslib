...
//    def kubectl = getKubectl(k8sEnv)
    ...
    node {
        ...
        try {
            ...
            def image_name = "image.rightcloud.com.cn/${namespace}/rightcloud-console-bigscreen:v.${branchname}.${env.BUILD_NUMBER}"
            def image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-console-bigscreen:v.${branchname}.${env.BUILD_NUMBER}"
            ...
            stage("build") {
                ...
                parallel(
                        failFast: true,
                        "clone console": {
                            dir('/home/jenkins/workspace/rightcloud-console-bigscreen') {
...
//                }
                stage('build bigscreen console image') {
...
//                            " --build-arg BACKUP_PROJECT=./rightcloud-console-cloudbackup "
                    def build_args = " --network=host  --build-arg CONSOLE_PROJECT=./rightcloud-console-bigscreen "
                        ...
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            dir('/home/jenkins/workspace') {
                                ...
                                if (params.BUILD_CPU_ARCH=="ARM64"){
                                    ...
                                    sh "docker build -t ${image_name_huaweiyun} --no-cache -f rightcloud-console-bigscreen/docker/Dockerfile.arm64 . ${build_args}"
                                ...
                                // if (params.BUILD_CPU_ARCH=="AMD64"){
                                //     sh "docker build --platform=linux/amd64 -t ${image_name} --no-cache -f rightcloud-console-bigscreen/docker/Dockerfile . ${build_args}"
                                //     sh "docker push ${image_name}"
                                ...
                                //     sh "echo '>>>>>>>>>AMD64镜像构建完成并上传成功，AMD镜像号为：${image_name}'"
                                //     sh "docker run --rm --privileged multiarch/qemu-user-static:register"
                                //     sh "docker build -t ${image_name} --no-cache -f rightcloud-console-bigscreen/docker/Dockerfile.arm64 . ${build_args}"
                                //     sh "docker push ${image_name}"
                                //     sh "echo '>>>>>>>>>ARM64镜像构建完成并上传成功，ARM镜像号为：${image_name}'"
            ...
            if (isDeploy) {
                stageK8ServiceDeploy("console-bigscreen", namespace, kubectl, image_name, false)
            }
