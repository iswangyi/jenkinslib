...
//    def kubectl = getKubectl(k8sEnv)
    node {
        ...
        // 获取Dockerfile 和 db-resource
         ...
         def image_name_huaweiyun = "swr.cn-east-3.myhuaweicloud.com/cmp/${namespace}/rightcloud-dbmigration:v.${branchname}.${env.BUILD_NUMBER}"
                        ...
                        container('docker') {
                            // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                                ...
                                if (params.BUILD_CPU_ARCH=="ARM64"){
                                    ...
                                    sh "docker build --network=host  -t ${image_name_huaweiyun} --no-cache -f docker/Dockerfile.arm64 . "
