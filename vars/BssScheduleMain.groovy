...
def call(isDeploy = true) {
    ...
    node {
        ...
        try {
            ...
            def image_name = "image.rightcloud.com.cn/${namespace}/bss-schedule:v.${branchname}.${env.BUILD_NUMBER}"

            stage("build") {
                // 获取Dockerfile
                parallel(
                    "clone schedule": {
                        dir('/home/jenkins/workspace/rightcloud-schedule') {
                ...
                def  build_args = " --network=host --build-arg   SCHEDULE_PROJECT=./rightcloud-schedule " +
                ...
                container('docker') {
                    dir('/home/jenkins/workspace') {
                        // withDockerRegistry([credentialsId: "02ffbd93-1dfc-4f77-b3be-1b403c9632e0", url: "https://image.rightcloud.com.cn"]) {
                            sh "docker build -t ${image_name} --no-cache -f rightcloud-schedule/docker/Dockerfile --no-cache . ${build_args}"
            ...
            if (isDeploy && params.IS_DEPLOY == "true") {
                stageK8ServiceDeploy("bss-schedule", namespace, kubectl, image_name, false, 'bss')
            }
        } catch (e) {
