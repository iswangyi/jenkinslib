...
//                },
                "build & deploy": {
                    ...
                    def runtime_image_name = "imagev2.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}-runtime:v.${branchname}.${env.BUILD_NUMBER}"
                    def image_name = "imagev2.rightcloud.com.cn/${namespace}/rightcloud-monitor/${SERVER_OR_AGENT}:v.${branchname}.${env.BUILD_NUMBER}"
                        ...
                        container('docker') {
                            withDockerRegistry([credentialsId: "harbor_buildarch", url: "https://imagev2.rightcloud.com.cn/"]) {
                                ...
                                if (params.BUILD_CPU_ARCH=="ARM64"){
                                    ...
                                    sh "docker build -t ${image_name} --no-cache -f monitor-${SERVER_OR_AGENT}/docker/Dockerfile.arm64 . ${build_args}"
                                    sh "docker push ${image_name}"
                                    sh "echo '>>>>>>>>>ARM64镜像构建完成并上传成功，ARM镜像号为：${image_name}'"
                                }

                                if (params.BUILD_CPU_ARCH=="AMD64"){
                                    sh "docker build -t ${image_name} --target mainjar --no-cache -f  monitor-${SERVER_OR_AGENT}/docker/Dockerfile.light .  ${build_args}"
                                    sh "docker push ${image_name}"
                                    sh "echo '>>>>>>>>>lgtag:应用包AMD64镜像构建完成并上传成功，应用包AMD镜像号为：${image_name}'"
                                    sh "docker build -t ${runtime_image_name} --target runtime -f monitor-${SERVER_OR_AGENT}/docker/Dockerfile.light . ${build_args}"
                                    sh "docker push ${runtime_image_name}"
                                    sh "echo '>>>>>>>>>lgtag:运行时AMD64镜像构建完成并上传成功，运行时AMD镜像号为：${runtime_image_name}'"
                                }

                                if (params.BUILD_CPU_ARCH=="AMD64_ARM64"){
                                    sh "docker build --platform=linux/amd64 -t ${image_name} --no-cache -f monitor-${SERVER_OR_AGENT}/docker/Dockerfile . ${build_args}"
                                    sh "docker push ${image_name}"
